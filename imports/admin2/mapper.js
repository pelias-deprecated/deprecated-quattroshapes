...
mapper._write = function( data, enc, next ){
  ...
  try {
    ...
    // Validation
    if( !data.properties.qs_a2_lc ){
      throw new Error( 'MAPPER - INVALID ID' );
    }

    if( !data.properties.qs_a2 ){
      throw new Error( 'MAPPER - INVALID NAME' );
    }

    this.push({
      _index: 'pelias', _type: 'admin2', _id: data.properties.qs_a2_lc,
      data: {
        ...
        suggest: {
          input: data.properties.qs_a2,
          output: data.properties.qs_a2,
          payload: {
            type: 'admin2',
